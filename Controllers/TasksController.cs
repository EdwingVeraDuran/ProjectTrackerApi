...
public class TasksController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<TaskResponseDto>> Create(Guid projectId, [FromBody] CreateTaskDto dto)
    {
        var userId = GetUserId();
        if (userId == null) return Unauthorized();
    ...
    [HttpPut("{taskId}")]
    public async Task<ActionResult<TaskResponseDto>> Update(Guid projectId, Guid taskId, [FromBody] UpdateTaskDto dto)
    {
        var userId = GetUserId();
        if (userId == null) return Unauthorized();
